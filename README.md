...
Jogo educativo desenvolvido integralmente em Python, com foco no ensino de conceitos de lógica proposicional de forma interativa, progressiva e gamificada. O projeto simula uma nave interestelar comprometida por um vírus lógico, onde o jogador assume o papel de tripulante para restaurar os sistemas.
...
O sistema segue uma arquitetura modular para garantir alta coesão e facilidade de manutenção.
...
└── fases/                 <- Conteúdo temático e bancos de perguntas 
    ...
    ├── implicacao.py      <- Módulo 3: Implicação Lógica (Argumentos) 
    ├── equivalencia.py    <- Módulo 4: Equivalência Lógica (De Morgan) 
...
## 🎮 Como Jogar
 1. **Teste de Admissão**: O jogador deve traduzir corretamente uma condicional (P \rightarrow Q) para avançar.
